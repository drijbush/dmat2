...
Contains
  ...
  Subroutine test_ks_split_matmul_NN
    ...
    Call Am%create( m, k, base )
    Call Am%print_info( 'Am - the source matrix', 9999 )
    Call Am%split( 2.0_wp, Am_split )
    Call Am_split%print_info( 'Am_split - the split matrix', 9999 )


    Call Bm%create( k, n, Am_split )
    Call Bm%print_info( 'Bm - the derived matrix', 9999 )
    Do spin = 1, ns
       Do kpoint = 1, nk
          If( k_types( kpoint ) == K_POINT_REAL ) Then
             Call Am_split%set_by_global( spin, k_points( :, kpoint ), 1, m, 1, k, A_r( :, :, kpoint, spin ) )
             Call Bm%set_by_global( spin, k_points( :, kpoint ), 1, k, 1, n, B_r( :, :, kpoint, spin ) )
          Else
             Call Am_split%set_by_global( spin, k_points( :, kpoint ), 1, m, 1, k, A_c( :, :, kpoint, spin ) )
             Call Bm%set_by_global( spin, k_points( :, kpoint ), 1, k, 1, n, B_c( :, :, kpoint, spin ) )
          End If
       End Do
    End Do
